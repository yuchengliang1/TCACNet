...
def extract_slices(inputs, batch_size, left_coord, device):
    ...
    for i in range(batch_size):
        sliced = inputs[i].narrow(1, left_coord[i, 0], 20)
...
def extract_features(inputs, model_local, batch_size, k_indexes, feature_w, n_slices, device):
    ...
    assert concat_slices.shape == (batch_size * n_slices, 1, 20, 199)
...
def replace_features(global_features, local_features, replace_idxs, device):
    ...
    #print(global_features.size())  # 查看 global_features 的形状
    flat_global_features = global_features.view(-1)
    #print(flat_global_features.size())  # 查看展平后的 global_features 形状

    flat_local_features = [i.view(-1) for i in local_features]
    flat_local_features = torch.cat(flat_local_features, 0)
    #print(flat_local_features.size())  # 查看展平后的 local_features 形状
...
def inference(inputs, wpser, model_global, model_local, model_top, n_slices, device, only_global_model, is_training):
    ...
    with torch.enable_grad():
        conv2_weight = model_global.conv2.weight.reshape(-1, 20)
    ...
    local_features, local_indexes = extract_features(inputs, model_local, batch_size, top_k_indices, feature_w,
                                                     n_slices, device)
    merged, flat_global_replaced, flat_local_features = replace_features(global_features, local_features, local_indexes,
                                                                         device)
